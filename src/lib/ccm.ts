...
export function ccmRules(): CcmRule[] {
  ...
  const rules = ccmControls.map((c): CcmRule => {
    ...
    const rule: CcmRule = {
      ...
      description: `Continuously evaluates "${ruleName(c)}" across the full population from the ${feedFor(c)} feed — no analyst re-keys results; evidence is captured automatically on every run.`,
      evidenceIds,
    }
    ...
    if (isPatchFail) {
      ...
      rule.description =
        'Continuously checks every asset in scope against the 14-day critical-patch SLA using the Qualys VM feed. On failure it auto-captures evidence, opens a remediation Issue and links the active incident — no analyst re-keys anything.'
    }
    return rule
  })
