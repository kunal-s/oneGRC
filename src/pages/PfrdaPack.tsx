...
import {
  CalendarClock, Users, ShieldCheck, Siren, FileText, ArrowUpRight, Download, Layers,
} from 'lucide-react'
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          <>
            The pension-specific obligation set — periodical returns, committee cadence, exposure-limit controls and
            ICS incident reporting — <span className="font-medium text-foreground">configured, not bolted on</span>.
          </>
        }
        actions={
        ...
        <Section title="ICS incident reporting" icon={<Siren className="size-4 text-critical" />} sub="quarterly · 48-hour · annual">
          <div className="space-y-2">
            ...
            <div className="rounded-md border border-border bg-muted/30 px-2.5 py-2 text-2xs text-muted-foreground">
              ICS taxonomy (Critical/High/Medium/Low) per circular PFRDA/2025/05/ICS/01 — incidents auto-classify and open the right regulator tracks.
            </div>
          </div>
        </Section>
      </div>
      ...
      <div className="mt-4 card-surface p-4">
        <h3 className="mb-3 flex items-center gap-1.5 text-sm font-semibold text-foreground">
          ...
          <span className="ml-2 inline-flex items-center gap-1 text-2xs font-normal text-muted-foreground">
            <Layers className="size-3" /> configured for SPF
          </span>
        </h3>
      ...
      <div className="mt-3 text-center text-2xs text-muted-foreground">
        Pension-specific content shipped with OneGRC — not a generic GRC template retrofitted to PFRDA.
      </div>
    </div>
  )
}
