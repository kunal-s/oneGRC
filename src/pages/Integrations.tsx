...
const PRECEDENTS = ['AEON Bank', 'IDFC First Bank', 'Citizens Business Bank']
...
export function Integrations() {
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Integrations · One platform view"
        title="Integrations — backbone &amp; spokes"
        description="OneGRC is the vendor-neutral, J2W-architected GRC/IRM backbone at the centre. Every security, compliance and pension system plugs in as a spoke — including the customer's existing in-house ITSM, which is kept and integrated, not replaced."
        actions={
          <button
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
        ...
        <span className="rounded-md border border-border bg-background px-2.5 py-1">
          Backbone: <span className="font-medium text-foreground">vendor-neutral GRC/IRM</span>
        </span>
      </div>
      ...
      <div className="card-surface overflow-hidden p-2">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 560 }}>
          ...
          <g className="animate-fade-in" style={{ animationDelay: '200ms' }}>
            ...
            <text x={CX} y={CY - 9} textAnchor="middle" fontSize={10.5} fill="hsl(210 40% 86%)">
              Unified GRC / IRM backbone
            </text>
            <text x={CX} y={CY + 7} textAnchor="middle" fontSize={9.5} fill="hsl(210 40% 72%)">
              vendor-neutral · J2W-architected
            </text>
          ...
          {SPOKES.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                {s.prominent && (
                  <g transform={`translate(${x + 14}, ${y + 58})`}>
                    <rect x={0} y={0} width={150} height={14} rx={7} fill="hsl(199 89% 95%)" />
                    <text x={6} y={10.5} fontSize={8} fontWeight={600} fill="hsl(199 80% 30%)">
                      KEPT — not replaced
                    </text>
                  </g>
                )}
      ...
      {/* slogan — the two retired GRC tools, NOT the ITSM */}
      <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <div className="rounded-xl border border-border bg-gradient-to-br from-primary to-[hsl(222_47%_18%)] p-5 text-primary-foreground">
          <div className="text-lg font-semibold tracking-tight">One platform instead of two.</div>
          <p className="mt-1.5 max-w-2xl text-sm text-primary-foreground/85">
            OneGRC retires the <span className="font-medium text-white">two disconnected GRC tools</span> — one for
            IT/security risk, one for enterprise &amp; regulatory compliance — and puts every risk, control, obligation,
            incident, policy and piece of evidence on a single data model. The in-house ITSM
            (<span className="font-medium text-white">Sankalp ServiceDesk</span>) is <span className="font-medium text-white">not</span> one
            of the two — it stays as the system of record and feeds the governance layer.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Pill>One taxonomy</Pill>
            <Pill>One calendar</Pill>
            <Pill>One evidence trail</Pill>
            <Pill>Every regulator, one clock</Pill>
          </div>
        </div>
        ...
        <div className="card-surface p-4">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <h3 className="text-sm font-semibold text-foreground">BFSI precedents</h3>
          </div>
          <p className="mb-2 text-2xs text-muted-foreground">
            IT-GRC and enterprise GRC unified on one backbone — already done in regulated banking.
          </p>
          <div className="space-y-1.5">
            {PRECEDENTS.map((p) => (
              <div key={p} className="flex items-center gap-2 rounded-md border border-border bg-background px-2.5 py-1.5">
          ...
          <div className="mt-3 flex items-center gap-1.5 text-2xs text-muted-foreground">
            <ShieldCheck className="size-3.5 text-ok" /> Backbone selection remains the customer's decision.
          </div>
        </div>
      </div>

      <div className="mt-3 flex items-center justify-center gap-1.5 text-2xs text-muted-foreground">
        Click any spoke to jump to where its data lands in OneGRC
